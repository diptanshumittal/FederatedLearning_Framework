...
class InvertedResidual(nn.Module):
    ...
    def forward(self, x, local_exp_ratio=None):
        residual = x
        if local_exp_ratio is not None and local_exp_ratio == self.exp_ratio:
            local_exp_ratio = None
        if local_exp_ratio is not None:
            assert local_exp_ratio <= self.exp_ratio
            new_mid_chs = make_divisible(self.conv_pw.in_channels * local_exp_ratio)
            mid_chs = self.conv_pw.out_channels
            mask = torch.zeros(1, mid_chs, 1, 1).to(x.device)
            mask[0, :new_mid_chs, 0, 0] = 1
