...
from HardCoreNAS.external.nas_parser import *
from HardCoreNAS.nas.nas_utils.general_purpose import extract_structure_param_list, target_time_loss, \
    freeze_weights_unfreeze_alphas, get_stage_block_from_name, STAGE_BLOCK_DELIMITER, OptimLike, \
    update_alpha_beta_tensorboard
from HardCoreNAS.nas.src.optim.block_frank_wolfe import flatten_attention_latency_grad_alpha_beta_blocks
...
from HardCoreNAS.timm.utils_new.cuda import ApexScaler, NativeScaler
...
def main():
    ...
    # print(list_alphas)
    # print(type(list_alphas))
    # import pickle
    # with open('list_alphas.pkl', 'wb') as outp:
    #     pickle.dump(list_alphas, outp, pickle.HIGHEST_PROTOCOL)
    print(fixed_latency)
...
def extract_grads(list_alphas, agg_steps=0, zero_grad=False, verbose=False):
    ...
    for e, entry in enumerate(list_alphas):
        ...
        name = get_stage_block_from_name(name, splitted=False) if key == 'alpha' else get_stage_block_from_name(name)[0]
